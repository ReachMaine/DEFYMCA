...
<div id="tribe-events-content" class="tribe-events-single">
	...
	<?php while ( have_posts() ) :  the_post(); ?>
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			<?php if ( function_exists( 'has_post_thumbnail' ) && has_post_thumbnail( $event->ID ) ) { // zig
				$has_image = true;
				echo '<div class="row">'; // begin row
				echo '<div class="large-8 columns">'; // begin column for details
			} ?>
			...
			<?php if ($has_image) { //* zig 
				echo '</div>';	// end column for details
				echo '<div class="large-4 columns">';
				/* <!-- Event featured image, but exclude link --> */
				echo tribe_event_featured_image( $event_id, 'full', false );
				echo '</div>';	// end column for image
				echo '</div>'; // end row
